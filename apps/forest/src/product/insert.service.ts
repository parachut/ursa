...
export class BPService {
  ...
  async insertItem(bhUrl, bhPrice): Promise<Product[]> {
    ...
    try {
      ...
      const result = await page.goto(bhUrl, {
      ...
      try {
        data = await page.evaluate(() => {
          //CAMERA
          const cameraString = $('h1[class^="title"]')
            .first()
            .text();
          ...
          //CAMERA PRICE/POINTS
          const cameraPriceSearch = $('*[data-selenium^="pricingPrice"]')
            .first()
            .text();
          ...
          if (
            imagesHtml === 'https://static.bhphoto.com/images/en/na500x500.jpg'
          ) {
            imagesHtml = '';
          }
          //IMG ID
          ...
          const categoryParentString = $('a[class^="linkCrumb_"]')
            .eq(-2)
            .text();
          const categoryParent = categoryParentString
          ...
          const categoryGrandParentString = $('a[class^="linkCrumb_"]')
            .eq(-3)
            .text();
          const categoryGrandParent = categoryGrandParentString
            ? categoryGrandParentString
            : '';
          ...
          //MFR
          const mfrString = $('*[class^="code"]')
            .first()
            .text();
          const mfrSplit = mfrString ? mfrString.split('•') : '';
          const mfr = mfrSplit[1].startsWith(' MFR')
            ? mfrSplit[1].replace(' MFR #', '')
            : '';
      ...
      try {
        dataSpecs = await page.evaluate(() => {
          ...
          const shutterIndex = values.findIndex(m =>
          ...
          if (!weightSearch1) {
            ...
            if (
              weightSearch1.value.includes('oz /') === true ||
              weightSearch1.value.includes('oz ') === true
            ) {
              weight = Math.ceil(parseFloat(weightSearch1.value) * 28.35);
            } else if (
              weightSearch1.value.includes('lb /') === true ||
              weightSearch1.value.includes('lb ') === true
            ) {
          ...
          if (weight === null) {
            const weightSearch2 = specifications.find(
              o => o.name === 'Package Weight',
            );
          ...
          for (const variation of dimensionsOption) {
            if (length === null && width === null && height === null) {
              const dimensions = specifications.find(
                o => o.name === `${variation}`,
              );
              if (!dimensions) {
                width;
                ...
                if (dimensions.value.endsWith('cm')) {
                  ...
                  const dim1 = dimensions.value.includes('/')
          ...
          if (length === null && width === null && height === null) {
            ...
            for (const variation of dimensionsOptionLens) {
              if (length === null && width === null && height === null) {
                let dimensions = null;
                dimensions = specifications.find(o =>
                  o.name.startsWith(`${variation}`),
                );
        ...
        if (data.general.name.includes('Kit') != true) {
          if (data.general.name.includes(' with') != true) {
            if (data.general.name.includes('Bundle') != true) {
              ...
              newItem = {
                general: { ...data.general, ...dataSpecs.general },
                brand_info: { ...data.brand_info },
                category_info: { ...data.category_info },
                specs: { ...dataSpecs.specs },
              };
              console.log(newItem);
                    ...
                    if (specifications.length != 0) {
                      try {
                        for (const specification of specifications) {
                          ...
                          await this.productAttributeValueRepository
                            .create({
                        ...
                        this.logger.error(
                          `Inserted to DB (SEQUELIZE) (SPECS) `,
                          e.stack,
                        );
                      }
                    }
                    this.logger.log(
                      `Inserted to DB (SEQUELIZE) `,
                      newRecord.id,
                    );
                  });
              } catch (e) {
