          ...
          await this.subscriptionRepository.update({
            ...
            .then(async record => {
              ...
              if (record[0] === 0) {
                ...
                if (subscription.couponRedemptions != null && subscription.couponRedemptions.length != 0) {
                ...
                if (subscription.addOns.length != 0 && subscription.addOns != null) {
                  const subAddOns = subscription.addOns?.map(async addon => {
                    await this.subscriptionAddOnRepository.create({
                      id: addon.id,
                      object: addon.object,
                ...
                if (subscription.addOns.length != 0 && subscription.addOns != null) {
                  const subAddOns = subscription.addOns?.map(async addon => {
                    ...
                    if (addon.addOn.id) {
                      try {
                        await this.subscriptionAddOnRepository.update({
                          object: addon.object,
                          quantity: addon.quantity,
                          unitAmount: addon.unitAmount,
                          createdAt: addon.createdAt,
